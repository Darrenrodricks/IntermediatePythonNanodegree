        ...
        def wrapper(*args, **kwargs):
            ...
            for arg,val in bound_arguments.items():
                ...
                if not isinstance(val, expected[arg]):
                    message(f"Bad Argument! Recieved {arg} = {val}, expecting object of type {expected[arg]}")
            retval = function(*args, **kwargs)
            if 'return' in expected and not isinstance(retval, expected['return']):
                message(f"Bad Return Value! Recieved {retval}, but expected object of type {expected[arg]}")
            return retval
        return wrapper
    return checker
